...
def main():
    ...
    def send():
        ...
        if msg != '':
            ...
            df_simi = pd.DataFrame(
                df, columns=['context', 'text response', 'similarity_bow'])
            print('done df simi')

            # sort the values to get answer to *most similar question*
            print('doing df simi_sort')
            df_simi_sort = df_simi.sort_values(
                by='similarity_bow', ascending=False)
    ...
    ChatLog.insert(
        END, "Annie: Welcome. My name is Annie, the Anxiety Bot. Just say whatever comes to mind." + '\n\n')
